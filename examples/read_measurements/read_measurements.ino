...
void loop() {
  ...
  if (info->valid) {
    ...
    Serial.printf("\nMode: %s", mppt.getModeString(data->operatingMode).c_str());
    
    Serial.printf(" (%s)", mppt.getChargeModeString(data->chargingMode).c_str());
    
    Serial.println();

    Serial.printf("Today: %.2f kWh\n", data->dayPower / 1000.0);
    Serial.printf("Total: %.2f kWh\n", data->totalPower / 1000.0);
